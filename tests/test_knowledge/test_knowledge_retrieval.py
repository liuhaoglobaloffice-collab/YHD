...
from src.database.models import CompanyBrainEntityModel, MemoryModel
...
@pytest.fixture
async def retrieval_service(async_session):
    ...
    entities = [
        await entity_repo.create(
            CompanyBrainEntityModel(
                ...
                attributes={"material": "plastic", "capacity": "500ml"},
            )
        ),
        await entity_repo.create(
            CompanyBrainEntityModel(
                ...
                attributes={"region": "ASEAN", "growth_rate": "15%"},
            )
        ),
    ]
    await async_session.commit()
...
async def test_search_memories(retrieval_service, admin_user, test_memories, async_session):
...
async def test_search_entities(retrieval_service, admin_user, test_entities, async_session):
...
async def test_search_with_pagination(retrieval_service, admin_user, test_memories, async_session):
...
async def test_search_without_permission_fails(retrieval_service, regular_user):
    ...
    retrieval_service.rbac.check_permission = AsyncMock(return_value=False)
...
async def test_context_summary(retrieval_service, admin_user, test_memories):
    ...
    assert len(summary) > 0  # Summary contains results or "No relevant knowledge found"
    assert "[memory]" in summary or "knowledge" in summary
...
async def test_search_creates_audit_log(
    ...
    result = await async_session.execute(
        ...
        .where(AuditLog.resource_type == "knowledge")
...
async def test_context_build_creates_audit_log(
    ...
    result = await async_session.execute(
        select(AuditLog)
        .where(AuditLog.action == "read")
        .where(AuditLog.resource_type == "knowledge_context")
        .order_by(AuditLog.timestamp.desc())
        .limit(1)
    )
    audit = result.scalar_one_or_none()
...
async def test_full_knowledge_retrieval_workflow(
    ...
    summary = context.get_summary()
    assert "packaging" in summary.lower() or "Found" in summary
